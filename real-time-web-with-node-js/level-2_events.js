 ...
 * 2. Next, let's listen for the 'message' event on our new chat object. 
...
// Emit events here
chat.emit('join', 'Diego');

// Now emit the 'message' event on the chat object. Just like before, remember 
// to pass in a custom message as a string.
chat.emit('message', 'Join us');


/**----------------------------------------------------------------------------
 * 2.4 Request Event
 * ----------------------------------------------------------------------------
 * Add an event listener on the server variable that listens to the request 
 * event. The event listener should take a callback function with two arguments, 
 ...
 * callback to your new 'request' event listener. Remember to remove the 
 * http.createServer() callback once the code has been moved.
 * 
 ...
 * 2. From inside of the new handler, log the message "New request coming in..."
 * using console.log().
*/
 ...
 * ----------------------------------------------------------------------------
 * Like our parents always used to say, listening is more important than 
 * talking! Modify the server so that we know when it's closed down.
 * 
